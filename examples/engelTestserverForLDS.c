...
writeServerCaps(UA_Server *server) {
 //   splitCaps();
    UA_NodeId myServerCapsNodeId = UA_NODEID_NUMERIC(0, UA_NS0ID_SERVERCONFIGURATION_SERVERCAPABILITIES);
    UA_Variant serverCapsVariant;
    UA_Variant_init(&serverCapsVariant);
    const char *delim = ":";
    int len = strcspn(serverCaps, delim);
    
    UA_String *serverCapabilities = (UA_String *)UA_Array_new(len, &UA_TYPES[UA_TYPES_STRING]);
    int count = 0;
    ...
    while (token2 != NULL) {
      ...
      count++;
      printf("%s\n", token2);
      token2 = strtok_r(NULL, delim, &inner_saveptr); 
    }
    UA_Variant_setArray(&serverCapsVariant, serverCapabilities, count, &UA_TYPES[UA_TYPES_STRING]);
...
int main(int argc, char* argv[]) {
    ...
    initialConfig.applicationDescription.applicationName = UA_LOCALIZEDTEXT_ALLOC("de", appName);
